...
function returnMenuName($get)
{
    db_connect();
    $query = "SELECT name FROM pages WHERE url = '$get' ";
    $result = mysql_query($query);
    $result = db_result_to_array($result);
...
function orderbycount($get)
{
    db_connect();
    $query = "SELECT * FROM photos
	WHERE photos.idlib = '$get'";
    $result = mysql_query($query);
    $result = db_result_to_array($result);
	$out1 = count($result);
	echo $out1;
...
{
    db_connect();
    $query = "SELECT name, pages.name FROM doc 
...
function foreachDeletePacket($get)
{
    db_connect();
...
//    $query = "SELECT libfile FROM photos WHERE photos.idlib = '150218(01,2,kruszwica)' ";
    ...
    foreach ($result as $filename) {
    	$filenameFull = 'files/' . $filename[0];
    	// echo '</br>' . $filenameFull . '</br>';
		unlink($filenameFull);    	
    }
 //    echo 'FILES AND SQL RECORDS ARE DELETED';
    return $result;
...
db_connect();
$result = mysql_query("
DELETE FROM photos WHERE idlib='$in1'
");
}	
...
function resultArrayListMonth()
{
    db_connect(); $query = "SELECT php3month FROM photos GROUP BY php3month";
    $result = mysql_query($query); $result = db_result_to_array($result); return $result;   
}

function resultArrayListCompany()
{
    db_connect(); $query = "SELECT file3company FROM photos GROUP BY file3company";
    $result = mysql_query($query); $result = db_result_to_array($result); return $result;   
}
...
{	
db_connect();
	$query = "SELECT *, pages.name FROM photos 
	JOIN pages ON photos.file2type = pages.id
	WHERE photos.idlib = '$start'   
	ORDER BY photos.idlib;
	";
$result = mysql_query($query);
$result = db_result_to_array($result);
return $result;   
